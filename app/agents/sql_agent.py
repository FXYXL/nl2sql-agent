...
SYSTEM_PROMPT_READONLY = """你是一个专业的 NL2SQL 助手。
...
SYSTEM_PROMPT_WRITE = """你是一个专业的 NL2SQL 助手。
根据用户提供的数据库表结构和自然语言问题，生成可执行的 SQL 语句。
...
2. SQL 语句不要用 markdown 代码块包裹
3. 确保 SQL 语法正确，适用于 MySQL
4. 使用表中实际存在的列名，不要编造列名
5. 修改数据前请确认操作正确
6. DELETE 和 UPDATE 操作必须带 WHERE 条件
"""

_DANGEROUS_KEYWORDS = re.compile(
    r'\b(DROP|ALTER|CREATE|TRUNCATE|REPLACE|GRANT|REVOKE|MERGE|EXEC|EXECUTE)\b',
    re.IGNORECASE,
)
...
async def ask(question: str, allow_writes: bool = False) -> dict:
    schema = await get_database_schema()
    system_prompt = SYSTEM_PROMPT_WRITE if allow_writes else SYSTEM_PROMPT_READONLY
    ...
    safety_error = _is_safe_sql(sql, allow_writes=allow_writes)
    if safety_error:
    ...
    return {
        ...
        "error": error,
        "is_write": is_write,
        "affected_rows": rows[0][0] if is_write and rows else 0,
    }
